        ...
        <tbody>
            @foreach($peminjaman as $data)
                <tr>
                    ...
                    <td>{!! $data->uraian_perangkat !!}</td>
                    <td>{!! $data->tgl_peminjaman !!}</td>
                    <td>{!! $data->tgl_pengembalian !!}</td>
                    <td>{!! $data->keperluan !!}</td>
                    <td>
                        <a
                            href="{{URL::to('/peminjaman/'. $data->id)}}">
                            <i class="livicon" data-name="info" data-size="18" data-loop="true" data-c="#428BCA"
                                data-hc="#428BCA" title="view peminjaman"></i>
                        </a>
                        <a
                            href="{{ route('peminjaman.edit', collect($data)->first() ) }}">
                            <i class="livicon" data-name="edit" data-size="18" data-loop="true" data-c="#428BCA"
                                data-hc="#428BCA" title="edit peminjaman"></i>
                        </a>
                        <a href="{{ route('peminjaman.confirm-delete', collect($data)->first() ) }}"
                            data-toggle="modal" data-target="#delete_confirm"
                            data-id="{{ route('peminjaman.delete', collect($data)->first() ) }}">
                            <i class="livicon" data-name="remove-alt" data-size="18" data-loop="true" data-c="#f56954"
                                data-hc="#f56954" title="delete peminjaman"></i>
